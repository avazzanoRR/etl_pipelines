...
def transform(input_filepath: str, output_dir: str) -> str:
    ...
    try:
        ...
        # Convert 'preferred_address_date_added' and 'preferred_address_date_last_changed' to datetime format
        logging.info("Converting 'preferred_address_date_added' and 'preferred_address_date_last_changed' to datetime format.")
        data['preferred_address_date_added'] = pd.to_datetime(data['preferred_address_date_added'], errors='coerce', format='%m/%d/%Y')
        data['preferred_address_date_last_changed'] = pd.to_datetime(data['preferred_address_date_last_changed'], errors='coerce', format='%m/%d/%Y')

        # Keep only the most recent record for each constituent_id based on preferred_address_date_added
        logging.info("Keeping only the most recent record for each constituent_id based on preferred_address_date_added.")
        data = data.sort_values(by=['constituent_id', 'preferred_address_date_added'], ascending=[True, False]).groupby('constituent_id').first().reset_index().copy()
