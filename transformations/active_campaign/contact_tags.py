...
def transform(input_filepath: str, output_dir: str) -> str:
    ...
    try:
        ...
        logging.info("Converting the 'tag_applied_timestamp' column to datetime.")
        data = convert_tz_string_to_datetime(data, column='tag_applied_timestamp', timezone='America/Chicago')

        # Fill missing values in 'tag_applied_timestamp' with values from 'tag_created_datetime'
        logging.info("Filling NAs in 'tag_applied_timestamp' with values from 'tag_created_datetime'.")
        data['tag_applied_timestamp'] = data['tag_applied_timestamp'].fillna(data['tag_created_datetime'])

        # Drop rows where 'tag_id' is missing
        logging.info("Dropping rows with missing 'tag_id'.")
        data = data.dropna(subset=['tag_id'])
