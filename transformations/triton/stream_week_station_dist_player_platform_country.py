...
def transform(input_filepath: str, output_dir: str) -> str:
    ...
    try:
        ...
        # Replace all blanks/nulls in 'distributor' to the string 'N/A'
        logging.info("Converting blank and None values in 'distributor' to 'N/A'.")
        df['distributor'] = df['distributor'].replace("", pd.NA).fillna("N/A")

        # Replace all blanks/nulls in 'player' to the string 'N/A'
        logging.info("Converting blank and None values in 'player' to 'N/A'.")
        df['player'] = df['player'].replace("", pd.NA).fillna("N/A")

        # Replace all blanks/nulls in 'platform' to the string 'N/A'
        logging.info("Converting blank and None values in 'platform' to 'N/A'.")
        df['platform'] = df['platform'].replace("", pd.NA).fillna("N/A")
